...
int main()
{
    ...
    {
        int u,v,w;
        printf("Enter starting,ending vertex and edge weight : ");
        scanf("%d%d%d",&u,&v,&w);
        e[i].u=u;
        e[i].v=v;
        e[i].w=w;
    ...
    float total_time=(end-start)/CLOCKS_PER_SEC;
    printf("\nTotal time taken by kruskal's algorithm using bubble sort is : %f\n",total_time);
    start=clock();
    quicksort(e1,0,el-1);
    ...
    printf("\nTotal time taken by kruskal's algorithm using quick sort is : %f\n",total_time);
    for(int i=0;i<n-1;i++)
    {
        printf("%d %d %d\n",new_tree[i].u,new_tree[i].v,new_tree[i].w);
    }
}